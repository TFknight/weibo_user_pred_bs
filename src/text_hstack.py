# -*- coding: UTF-8 -*-
''' doc2vc-dm and dbow -svm stack for gender'''
...
class user_predict:
    ...
    def train_lsi_model(self,doc):
        ...
        #训练模型
        model_dm = Doc2Vec(X_doc,dm=1, size=200, negative=5, hs=0, min_count=1, window=30,sample=1e-5,workers=8,alpha=0.025,min_alpha=0.025)
        model_dbow = Doc2Vec(X_doc,dm=0, size=200, negative=5, hs=0, min_count=1, window=30,sample=1e-5,workers=8,alpha=0.025,min_alpha=0.025)
        joblib.dump(model_dm,"model_d2v_hstack_dm2.model")
        joblib.dump(model_dbow,"model_d2v_hstack_dbow2.model")
        print "d2w模型训练完成"
    ...
    def train_lsi(self,doc):
        ...
        if(os.path.exists("model_d2v_hstack_dm2.model")):

            #load train model
            model_dm = joblib.load("model_d2v_hstack_dm2.model")
            model_dbow = joblib.load("model_d2v_hstack_dbow2.model")
        else:
            #load train model
            model_dm,model_dbow = self.train_lsi_model(doc)
        ...
        for i in range(10):
            ...
            X_vec_dm = np.array([model_dm.docvecs[i] for i in range(len(list_total))])
        for i in range(15):
            model_dbow.train(X_doc, total_examples=model_dbow.corpus_count, epochs=2)
            X_vec_dbow = np.array([model_dbow.docvecs[i] for i in range(len(list_total))])

        print X_vec_dm.shape
        print X_vec_dbow.shape
        X_d2v = np.hstack((X_vec_dbow,X_vec_dm))
        print "total shape :     " + str(X_d2v.shape)
