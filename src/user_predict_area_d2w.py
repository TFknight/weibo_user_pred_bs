...
class user_predict:
    ...
    def train_lsi_model(self,doc):
        ...
        joblib.dump(model_dm,"model_d2v.model")
        print "d2w模型训练完成"
    ...
    def train_lsi(self,doc):
        ...
        if(os.path.exists("model_d2v.model")):

            #load train model
            model_dm = joblib.load("model_d2v.model")
        else:
            #load train model
    ...
    def predict(self):
        ...
        # gridsearch 找最优模型
        param_grid = {'C': [0.8, 1, 2, 3, 4, 5], 'kernel': ['ovo','ovr']}
        ...
        list_area = np.argmax(y_pred_te,axis=1)

        list_area_2 = []
        for i in list_area:
            list_area_2.append(i)
        print "list turn done"

        for i in set(list_area_2):
            print i,list_area_2.count(i)
        ...
        print "doc2vec + 多分类 + 支持向量机　准确率平均值为: "
        print  self.mymean(list_myAcc)


if __name__ == '__main__':

    user_predict = user_predict("area_train.csv","area_test.csv","area_valid.csv")
